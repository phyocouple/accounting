...
class PatientLabSample(models.Model):
    ...
    container_name = fields.Char(string='Sample Container Code', help="If using preprinted sample tube/slide/box no can be updated here.", copy=False, index=True, states=STATES)
    patient_test_ids = fields.Many2many('patient.laboratory.test', 'test_lab_sample_rel', 'sample_id', 'test_id', string="Patient Lab Tests", states=STATES)
    test_ids = fields.Many2many('acs.lab.test', 'acs_test_lab_sample_rel', 'sample_id', 'test_id', string="Lab Tests", states=STATES)
    ...
    def create(self, vals_list):
        for values in vals_list:
            values['name'] = self.env['ir.sequence'].next_by_code('acs.patient.laboratory.sample')
